...
```bash
pnpm install
cd packages/dev-session && pnpm build
```
...
```bash
pnpm session:stop 001   # Stop without destroying
pnpm session:start 001  # Start again
```

### Destroy a session

```bash
pnpm session:destroy 001     # Destroy specific session
pnpm session:destroy --all   # Destroy all sessions
```
