...
program
  ...
  .action(async () => {
    ...
    if (!existsSync(envFile)) {
      ...
      console.log(chalk.gray('Run `dev-session create --in-place` to create a session here.'));
      process.exit(1);
    }
...
program
  ...
  .action(async () => {
    ...
    console.log(`
${chalk.bold('dev-session')} - Manage isolated parallel development sessions

${chalk.bold('USAGE')}
  dev-session <command> [options]
...
${chalk.bold('MORE INFO')}
  Run ${chalk.cyan('dev-session <command> --help')} for command-specific options
`);
  });
